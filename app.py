...
def system_monitor_page():
    if not is_authenticated():
        return redirect(url_for('login'))
    return render_template('system_monitor_fixed.html')
...
def api_system_monitor_stats():
    try:
        ...
        for disk in psutil.disk_partitions():
            try:
                if disk.mountpoint in ['/', '/tmp', '/home']:  # Only show main partitions
                    usage = psutil.disk_usage(disk.mountpoint)
                    disk_stats[disk.mountpoint] = {
                ...
                disk_stats[disk.mountpoint] = {'error': True}
        ...
        return jsonify({
            ...
            'disk': disk_stats,
            'gpu': {'available': False, 'message': 'GPU monitoring disabled'},
        ...
        log_to_db('ERROR', f'System monitor API error: {str(e)}')
        return jsonify({
            'cpu_percent': 0,
            'ram_percent': 0,
            'ram_used_gb': 0,
            'ram_total_gb': 0,
            'disk': {},
            'gpu': {'available': False, 'error': 'System monitor error'},
            'error': str(e)
        }), 500
...
def api_gpu_status():
    ...
    try:
        # Try to import and use gpu_manager
        try:
            ...
            # Fallback if gpu_manager not available
            gpus = []
            allocation = {'whisper': 'CPU', 'ollama': 'CPU', 'strategy': 'لا توجد كروت شاشة متاحة'}
...
def api_gpu_refresh():
    ...
    try:
        # Try to refresh GPU information
        try:
            from gpu_manager import GPUManager
            gpu_manager = GPUManager()
        ...
        return jsonify({
            ...
            'gpus': gpus,
            'total_gpus': len(gpus),
            'message': 'تم تحديث معلومات كروت الشاشة' if success else 'لا توجد كروت شاشة NVIDIA متاحة'
        })
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500

@app.route('/api/gpu/smart-allocation', methods=['POST'])
def api_gpu_smart_allocation():
    """Apply smart GPU allocation"""
    if not is_authenticated():
        return jsonify({'error': 'Unauthorized'}), 401
    
    try:
        # Try to get optimal allocation
        try:
            from gpu_manager import GPUManager
            gpu_manager = GPUManager()
            allocation = gpu_manager.get_optimal_allocation()
        ...
        # Update settings
        update_setting('whisper_model_gpu', allocation['whisper'])
        update_setting('ollama_model_gpu', allocation['ollama'])
        
        return jsonify({
            'success': True,
            'allocation': allocation,
            'message': 'تم تطبيق التوزيع الذكي بنجاح'
        })
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
@app.route('/api/download-install-script')
def download_install_script():
    """Download automatic driver installation script"""
    try:
        script_path = os.path.join(os.getcwd(), 'install_drivers.sh')
        if os.path.exists(script_path):
            return send_file(script_path, 
                           as_attachment=True, 
                           download_name='install_drivers.sh',
...
@app.route('/api/system-install-guide')
def system_install_guide():
    """Get system installation guide with automatic script"""
    if not is_authenticated():
        return jsonify({'error': 'Authentication required'}), 401
    ...
    return jsonify({
        'success': True,
        'guide': {
            'title': 'دليل تثبيت التعريفات والأدوات التلقائي',
            'description': 'سكريبت تلقائي لتثبيت جميع التعريفات والأدوات اللازمة لتشغيل الترجمان الآلي',
            'features': [
                'تثبيت تعريفات NVIDIA وCUDA تلقائياً',
                'تثبيت Python وجميع المكتبات المطلوبة',
                'تثبيت FFmpeg لمعالجة الفيديو',
                'تثبيت PostgreSQL وNginx',
                'تثبيت Docker وOllama للذكاء الاصطناعي',
                'تكوين الجدران النارية والأمان'
            ],
            'usage': [
                'curl -O https://your-domain.com/api/download-install-script',
                'chmod +x install_drivers.sh',
                './install_drivers.sh'
            ],
            'download_url': '/api/download-install-script'
        }
    })
