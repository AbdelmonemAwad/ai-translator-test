...
from translations import get_translation, t, translate_text
from auth_manager import auth_manager
from database_setup import create_database_with_dummy_data
...
def get_user_language():
    ...
    return get_setting('default_language', 'en')  # Default to English as base language
...
def setup_page():
    ...
    if request.method == 'POST':
        ...
        if not username or len(username) < 3:
            ...
            return render_template('setup.html', error='Username must be at least 3 characters long')
        ...
        # Create admin user
        if auth_manager.create_initial_admin(username, password):
...
def settings_page():
    ...
    for setting in Settings.query.order_by(Settings.section, Settings.key).all():
        ...
        if setting.type == 'select' and setting.options:
            ...
            # Enhanced options parsing with Arabic translation
            if options_text:
                for option_pair in options_text.split(','):
                    ...
                    if ':' in option_pair:
                        ...
                        # Apply Arabic translations for common values
                        if value == 'true':
                            label = 'نعم / Yes'
                        elif value == 'false':
                            label = 'لا / No'
                        ...
                        # Format: just value - apply translations
...
def api_system_monitor_stats():
    try:
        from system_monitor import SystemMonitor, format_bytes, format_uptime
        
        # Initialize system monitor
        system_monitor = SystemMonitor()
        system_info = system_monitor.get_all_stats()
        ...
        stats = {
            'system': {
                'hostname': system_details.get('hostname', 'Unknown'),
                'os_name': f"{system_details.get('system', 'Unknown')} {system_details.get('release', '')}",
                'os_version': system_details.get('version', 'Unknown'),
                'uptime': format_uptime(system_details.get('uptime', 0)),
                'architecture': system_details.get('architecture', ['Unknown', ''])[0],
                'processor': system_details.get('processor', 'Unknown'),
                'users': system_details.get('users', 0),
                'processes': system_details.get('processes', 0)
            },
            'cpu': {
            ...
            'memory': {
                ...
                'swap_total': format_bytes(memory_info.get('swap_total', 0)),
                'swap_used': format_bytes(memory_info.get('swap_used', 0)),
                'swap_percentage': memory_info.get('swap_percentage', 0),
                'type': memory_info.get('type', 'Unknown'),
                'speed': memory_info.get('speed')
            },
            'disk': {
...
def download_github_release():
    ...
    release_files = [
        'ai-translator-v2.2.4.zip',          # Latest version (with all fixes)
        'ai-translator-v2.2.2.zip',          # Previous stable version
        'ai-translator-github-v2.2.1.zip',   # Legacy release
        'ai-translator-github-v2.2.0.zip'    # Fallback
    ]
    
    for release_file in release_files:
        if os.path.exists(release_file):
            if 'v2.2.4' in release_file:
                version = '2.2.4'
...
# Enhanced API Routes for Improved Functionality
@app.route('/api/gpu-status-enhanced')
def api_gpu_status_enhanced():
    """Get enhanced GPU status and options"""
    if not is_authenticated():
        return jsonify({'error': 'Unauthorized'}), 401
    
    try:
        gpu_info = gpu_manager.get_gpu_info()
        gpu_options = gpu_manager.get_gpu_options()
        ...
        return jsonify({
            ...
            'nvidia_available': gpu_info['has_nvidia'],
            'amd_available': gpu_info['has_amd'],
            'intel_available': gpu_info['has_intel']
        })
    except Exception as e:
        return jsonify({'error': str(e)}), 500

@app.route('/api/gpu-refresh-enhanced', methods=['POST'])
def api_gpu_refresh_enhanced():
    """Refresh GPU detection"""
    if not is_authenticated():
        return jsonify({'error': 'Unauthorized'}), 401
    
    try:
        from gpu_manager import refresh_gpu_detection
        gpu_info = refresh_gpu_detection()
        
        return jsonify({
            'success': True,
            'message': 'GPU detection refreshed',
            'gpu_info': gpu_info
        })
    except Exception as e:
        return jsonify({'error': str(e)}), 500

@app.route('/api/database-initialize-enhanced', methods=['POST'])
def api_database_initialize_enhanced():
    """Initialize database with comprehensive dummy data"""
    if not is_authenticated():
        return jsonify({'error': 'Unauthorized'}), 401
    
    try:
        success = create_database_with_dummy_data()
        
        if success:
            return jsonify({
                'success': True,
                'message': 'Database initialized with comprehensive dummy data'
            })
        else:
            return jsonify({
                'success': False,
...
@app.route('/api/settings-structure')
def api_settings_structure():
    """Get settings structure for enhanced UI"""
    if not is_authenticated():
        return jsonify({'error': 'Unauthorized'}), 401
    
    try:
        from database_setup import get_comprehensive_settings_structure
        structure = get_comprehensive_settings_structure()
...
def api_system_stats():
    """Simple and reliable system statistics API"""
    try:
        import time
